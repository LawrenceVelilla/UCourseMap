...
const courseGraphData = {
  "nodes": [
    ...
    // Prerequisite Nodes
    { "id": "CMPUT 175", "label": "CMPUT 175: Introduction to the Foundations of Computation II", "type": "prerequisite" },
    { "id": "CMPUT 272", "label": "CMPUT 272: Formal Systems and Logic in Computing Science", "type": "prerequisite" },
    { "id": "CMPUT 275", "label": "CMPUT 275: Introduction to Tangible Computing II", "type": "prerequisite" },
    ...
    { "id": "MATH 144", "label": "MATH 144: Calculus for the Mathematical and Physical Sciences I", "type": "prerequisite" },
    { "id": "MATH 154", "label": "MATH 154: Calculus for Business and Economics I", "type": "prerequisite" }
  ],
  "edges": [
...
const getLayoutedElements = (
    ...
    nodesToLayout.forEach((node) => dagreGraph.setNode(node.id, {
    ...
    const finalNodes: AppNode[] = nodesToLayout.map((node) => {
        ...
        // Apply default styles
         const style = node.style ?? (node.data.type === 'target'
                        ? { background: '#606c5d', color: '#fff', border: '1px solid #3a4139', borderRadius: '4px', width: nodeWidth, padding: '10px 15px', textAlign: 'center', fontSize: '13px', height: nodeHeight }
                        : { background: '#f0f0e8', color: '#333', border: '1px solid #d1d1c4', borderRadius: '4px', width: nodeWidth, padding: '10px 15px', textAlign: 'center', fontSize: '13px', height: nodeHeight });
     ...
     const finalEdges: AppEdge[] = edgesToLayout.map(edge => ({
        ...
        markerEnd: edge.markerEnd ?? { type: MarkerType.ArrowClosed, color: '#7d8a70', width: 18, height: 18 },
        type: 'smoothstep',
    }));
...
const FlowWithLayout = () => {
  ...
  useEffect(() => {
    ...
    if (transformedNodes.length > 0) {
        const layoutOptions = { direction: 'TB' as LayoutDirection };
...
export default function ClassesTestPage() {
  return (
    <div className="p-4 md:p-8 flex flex-col" style={{ height: 'calc(100vh - 100px)' }}> {/* Adjust height as needed */}
