...
/* To do
  ...
  Get all courses in data
  pass each course JSON to OpenAI
...
const basePrompt = `
  ...
  - Possible operators are: "AND", "OR", "STANDALONE", and "WILDCARD"
...
async function parseWithOpenAI(description: string): Promise<{
  ...
  const prompt = `${basePrompt} Input: ${description}
  `;
  ...
  // Error --> FIXXX (prerequisites does not exist in known requirements)
  try {
