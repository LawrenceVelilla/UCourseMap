  ...
  return (
    <>
      ...
      {operatorText && (
        ...
        >
          {" "}
          {/* Added inline-block, align-baseline, mr-1 */}
          {operatorText}
        </span>
      )}
            ...
            condition.courses!.map((itemText) => {
              ...
              return (
                <li key={itemText} className="m-1">
                  {" "}
                  {/* List item wrapper */}
                  {/* Add group, keep hover:bg, remove text colors */}
                  <span className="group pl-2 pr-2 rounded-md transition-colors duration-200 hover:bg-[#606c5d]">
                    {isCourse && parsedLinkData ? (
                      <Link
                        href={`/?dept=${parsedLinkData.dept.toLowerCase()}&code=${parsedLinkData.code}`}
                        // Add base text color and group-hover text color
                      ...
                      // Also add base text color and group-hover text color here
        ...
        <>
          {hasDirectCourses &&
            condition.courses!.map((itemText) => {
              ...
              return (
                <div className="m-1" key={itemText}>
                  {" "}
                  {/* Wrap in div/span if needed for key/styling */}
                  {isCourse && parsedLinkData ? (
                    <span className="group pl-2 pr-2 rounded-md transition-colors duration-200 hover:bg-[#606c5d]">
                      <Link
                        href={`/?dept=${parsedLinkData.dept.toLowerCase()}&code=${parsedLinkData.code}`}
                        className="text-[#588157] hover:text-white"
                        title={`Check prerequisites for ${itemText}`}
                      >
                        {itemText}
                      </Link>
