...
function parseCourseLi($, li) {
  const $li = $(li);
  const rawText = $li.find('span > a').first().text()?.trim().replace(/\u00A0/g, ' ') || null;
  const noteSpanText = $li.find('span').first().text()?.trim().replace(/\u00A0/g, ' ') || null;
  ...
  return fullCode ? { courseCode: fullCode, title: courseTitle, department: department, note: note } : null;
}
    ...
    cores.forEach(coreEl => {
      ...
      $core.children().each((j, child) => {
        ...
        if ($child.is('h3, h4')) {
          ...
          if (!currentGroup) { // Handle paragraph before any h3/h4
            currentGroup = { groupTitle: null, description: [], courses: [] };
            groups.push(currentGroup);
          }
          currentGroup.description.push($child.text().trim().replace(/\u00A0/g, ' '));
        } else if ($child.is('ul')) {
          // Process list items for the current group
          if (!currentGroup) { // Handle list before any h3/h4
            currentGroup = { groupTitle: null, description: [], courses: [] };
            groups.push(currentGroup);
          }
          $child.find('> li').each((k, li) => {
            ...
            if ($li.hasClass('acalog-course')) {
              ...
              currentGroup.description.push($li.text().trim().replace(/\u00A0/g, ' '));
            }
          });
        }
      ...
      const notesList = $core.find('ol > li')
                            .map((_, li) => $(li).text().trim().replace(/\u00A0/g, ' '))
                            .get();

      // Determine the representative title for this block (e.g., first group title)
      const blockTitle = groups.length > 0 ? groups[0].groupTitle : ($core.find('h3,h4').first().text().trim() || null);
